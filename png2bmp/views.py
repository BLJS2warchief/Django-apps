...
def index(request):
	...
	pngBitDepthInt = int.from_bytes(pngBitDepth, byteorder='big')
	...
	maskBitValue = 2 ** pngBitDepthInt - 1
	pixelSize = math.ceil(rgbA*pngBitDepthInt/8)
	extraBits = pixelSize * 8 - rgbA*pngBitDepthInt
	shiftBitsR = pngBitDepthInt * (rgbA - 1) + extraBits
	if (rgbA > 2):
		shiftBitsG = pngBitDepthInt * (rgbA - 2) + extraBits
		shiftBitsB = pngBitDepthInt * (rgbA - 3) + extraBits
	if (alphaFlag):
		shiftBitsA = pngBitDepthInt * (rgbA - rgbA) + extraBits

	print("rgbA", rgbA, "pngBitDepth", pngBitDepthInt, "extraBits", extraBits)
	print("maskBitValue", maskBitValue, "PixelSize", pixelSize, "shiftBitsR", shiftBitsR, "shiftBitsG", shiftBitsG, "shiftBitsB", shiftBitsB, "shiftBitsA", shiftBitsA)
	...
	# while(zlibPrevLoc == 0):
		if (plteFlag):
			zlibDataLoc += math.ceil(pngWidthInt*pngBitDepthInt/8)+1
		else:
			zlibDataLoc += math.ceil(pngWidthInt*rgbA*pngBitDepthInt/8)+1
		...
		while (i < pngWidthInt):
			...
			if (plteFlag):
				...
				if (extraBits > 0):
					ii = i%(8//extraBits)
				if (extraBits == 0 or ii == 0):
					indx = zlibScanLine[i*pngBitDepthInt//8+1]
					# print(zlibScanLine[i*pngBitDepthInt//8+1], bin(indx), indx, end='--, ')
				else:
					indx = zlibScanLine[i*pngBitDepthInt//8+1]
					# print(zlibScanLine[i*pngBitDepthInt//8], bin(indx), indx, end='--, ')
					indx = indx << (8-ii*extraBits)
				# print(bin(indx), indx, end='___, ')
				indx = indx >> (8 - pngBitDepthInt)
				indx = indx  & maskBitValue
				# print(bin(indx), indx, end='===')
				valR = plteR[indx]
				if (rgbA > 2):
					valG = plteG[indx]
					valB = plteB[indx]
				if (alphaFlag):
					valA = plteB[indx]		#### pltA_______
				i += 1
			else:
				if (extraBits > 0):
					ii = i%(8//extraBits)
				if (extraBits == 0 or ii == 0):
					pixelData = int.from_bytes(zlibScanLine[i*pixelSize+1:(i+1)*pixelSize+1], byteorder='big')
				else:
					pixelData = int.from_bytes(zlibScanLine[i*pixelSize:(i+1)*pixelSize+1], byteorder='big')
					pixelData = pixelData << (8-ii*extraBits)
					pixelData = pixelData >> 8
				valR = ((pixelData >> shiftBitsR) & maskBitValue).to_bytes(1, byteorder='little')
				if (rgbA > 2):
					valG = ((pixelData >> shiftBitsG) & maskBitValue).to_bytes(1, byteorder='little')
					valB = ((pixelData >> shiftBitsB) & maskBitValue).to_bytes(1, byteorder='little')
				if (alphaFlag):
					valA = ((pixelData >> shiftBitsA) & maskBitValue).to_bytes(1, byteorder='little')
				# valR = pixelData[0]
				# if (rgbA > 2):
				# 	valG = pixelData[1]
				# 	valB = pixelData[2]
				# if (alphaFlag):
				# 	valA = bytes(pixelData[3])
				# print(pixelData, end=', ')
				i += 1
			if (typeFilter == 1):
